...
from app.ui.mini_player import MiniPlayerWindow
...
from app.services.mpris import MprisService
...
class VoremWindow(Adw.ApplicationWindow):
    def __init__(self,**kwargs):
        ...
        self.queue=[];self.queue_index=-1;self.current_song_id=None;self.shuffle_history=[];self.shuffle_seen_ids=set()
        # Orden natural (tal como aparece en la lista) de la cola actual, sin
        # mezclar. Se usa para poder restaurar el orden real de la lista en
        # cuanto el aleatorio se desactiva, en vez de arrastrar el orden ya
        # mezclado que quedó de cuando el aleatorio estaba activo.
        self.queue_source=[]
        ...
        # Minireproductor flotante (Control+J): ventana pequeña independiente
        # con carátula, transporte y barra de progreso, sin el visualizador.
        self.mini_player=MiniPlayerWindow(self.engine,self._previous_song,self._next_song,self,self._toggle_mini_player)
        ...
        self.engine.connect("state-changed", lambda _e, state: (self.player_view.set_state(state), self._notify_mpris()))
        self.engine.connect("spectrum-updated", lambda _e, values: self.player_view.set_spectrum(values))
        self.mpris = MprisService(self)
        self.connect("close-request", lambda *_: (self.mpris.shutdown(), False)[-1])
        ...
        # Sincroniza los botones de aleatorio/repetir con el estado guardado
        # en la base de datos. Sin esto, si el aleatorio quedó activado en
        # una sesión anterior, el botón se ve apagado pero "Siguiente"
        # sigue eligiendo canciones al azar en vez de seguir el orden real.
        ...
        self._setup_actions()
        # Reintenta localizar automáticamente las carpetas guardadas al iniciar.
        # Si un disco aún no está montado, el escáner conserva la biblioteca y no pide credenciales.
        GLib.idle_add(self.library_page._on_scan, None)
    ...
    def _play_song(self, row, queue):
        ...
        # pertenece a la cola actual y la lista de origen no cambió, la
        # conservamos tal cual para no romper el aleatorio ya en curso.
        if self.queue and self.queue_source == incoming and any(r["id"] == row["id"] for r in self.queue):
            self.queue = list(self.queue)
        else:
            self.queue_source = incoming
            self.queue = list(incoming)
            self.shuffle_history.clear()
            self.shuffle_seen_ids.clear()
            if self.shuffle_enabled and len(self.queue) > 1:
                others = [r for r in self.queue if r["id"] != row["id"]]
                random.shuffle(others)
                self.queue = [row] + others
            self.shuffle_seen_ids.add(row["id"])
    ...
    def _load_queue_index(self, index, show_view=False):
        ...
        self.mini_player.set_song(row)
        self.player_bar.set_navigation_enabled(len(self.queue) > 1)
        self.mini_player.set_navigation_enabled(len(self.queue) > 1)
        if show_view:
    ...
    def _play_all(self, rows):
        ...
        if not rows:
            return
        self.queue_source = list(rows)
        self.shuffle_history.clear()
        self.shuffle_seen_ids.clear()
        # "Reproducir todo" debe empezar SIEMPRE por la primera canción de la
        # lista, incluso con el aleatorio activado: solo se mezcla el resto
        # de la cola (igual que ya hace _play_song al reproducir una pista
        # concreta), en vez de sortear también cuál es la primera en sonar.
        if self.shuffle_enabled and len(rows) > 1:
            first, others = rows[0], rows[1:]
            random.shuffle(others)
            self.queue = [first] + others
        else:
            self.queue = list(rows)
        if self.queue:
            self.shuffle_seen_ids.add(self.queue[0]["id"])
    ...
    def _set_shuffle(self, enabled):
        ...
        self.shuffle_history.clear()
        self.shuffle_seen_ids.clear()
        # Al cambiar el modo aleatorio, la cola se reconstruye siempre a
        # partir del orden real de la lista (queue_source) en vez de seguir
        # arrastrando el orden mezclado anterior. Así, apagar el aleatorio
        # vuelve de inmediato al orden en el que aparecen las canciones en
        # la lista, y encenderlo vuelve a mezclar desde ese mismo orden.
        if self.queue_source:
            current_id = self.current_song_id
            if self.shuffle_enabled and len(self.queue_source) > 1:
                current_row = next((r for r in self.queue_source if r["id"] == current_id), None)
                if current_row is not None:
                    others = [r for r in self.queue_source if r["id"] != current_id]
                    random.shuffle(others)
                    self.queue = [current_row] + others
                    self.queue_index = 0
                else:
                    self.queue = list(self.queue_source)
                    random.shuffle(self.queue)
                    self.queue_index = 0
            else:
                self.queue = list(self.queue_source)
                if current_id is not None:
                    idx = next((i for i, r in enumerate(self.queue) if r["id"] == current_id), None)
                    if idx is not None:
                        self.queue_index = idx
            if self.shuffle_enabled and self.queue and 0 <= self.queue_index < len(self.queue):
                self.shuffle_seen_ids.add(self.queue[self.queue_index]["id"])
            self.player_bar.set_navigation_enabled(len(self.queue) > 1)
            self.mini_player.set_navigation_enabled(len(self.queue) > 1)
    ...
    def _next_song(self):
        if not self.queue:
            return
        if self.shuffle_enabled and len(self.queue) > 1:
            candidates = [i for i, r in enumerate(self.queue)
                          if i != self.queue_index and r["id"] not in self.shuffle_seen_ids]
            if not candidates:
            ...
            self.shuffle_seen_ids.add(self.queue[next_index]["id"])
            self._load_queue_index(next_index, show_view=False)
            return
    ...
    def _on_global_key(self,_controller,keyval,_keycode,state):
        # Control+J abre/cierra el minireproductor flotante. Se comprueba
        # antes que cualquier otra cosa (incluida la exclusión de campos de
        # texto) para que el atajo funcione siempre, se esté escribiendo o no.
        ctrl=bool(state & Gdk.ModifierType.CONTROL_MASK)
        if ctrl and keyval in (Gdk.KEY_j,Gdk.KEY_J):
            self._toggle_mini_player();return True
        ...
        # También se excluyen los controles donde espacio/flechas ya tienen
        # su propio significado (deslizadores de progreso/volumen, listas de
        # canciones, botones, spinbuttons, combos...), para no robarles la
        # tecla mientras el usuario los está usando.
        if isinstance(focus,(Gtk.Editable,Gtk.TextView,Gtk.Range,Gtk.SpinButton,
                              Gtk.ListBox,Gtk.ListBoxRow,Gtk.Button,Gtk.ToggleButton,
                              Gtk.CheckButton,Gtk.Switch,Gtk.ComboBoxText,Gtk.DropDown,
                              Gtk.TreeView,Gtk.ColumnView)):
            return False
        # Barra espaciadora: reproducir/pausar. Flechas izquierda/derecha:
        # canción anterior/siguiente, igual que las teclas multimedia
        # dedicadas de abajo. Todo esto solo llega aquí cuando la ventana
        # tiene realmente el foco del teclado, es decir, cuando la app está
        # en primer plano (no minimizada ni detrás de otra ventana): si la
        # app pierde el foco, GTK deja de enviarle estos eventos.
        if keyval == Gdk.KEY_space:self.engine.toggle();return True
        if keyval in (Gdk.KEY_Left,Gdk.KEY_KP_Left):self._previous_song();return True
        if keyval in (Gdk.KEY_Right,Gdk.KEY_KP_Right):self._next_song();return True
        # A propósito SOLO se capturan (además de lo anterior) las teclas
        # multimedia dedicadas del teclado (XF86Audio...). El control real
        # "aunque la ventana no esté abierta" lo da el soporte MPRIS (ver
        # app/services/mpris.py), no esta captura local.
        if keyval == getattr(Gdk, "KEY_AudioNext", -999):self._next_song();return True
        if keyval == getattr(Gdk, "KEY_AudioPrev", -998):self._previous_song();return True
        if keyval == getattr(Gdk, "KEY_AudioPlay", -997):self.engine.toggle();return True
        if keyval == getattr(Gdk, "KEY_AudioPause", -996):self.engine.pause();return True
        if keyval == getattr(Gdk, "KEY_AudioStop", -995):self.engine.stop();return True
        return False

    def _toggle_mini_player(self, force_show_main=False):
        # Control+J alterna entre el reproductor normal (esta ventana) y el
        # minireproductor flotante: solo uno de los dos debe estar visible
        # a la vez. Se usa hide()/present() en vez de close() para que la
        # ventana principal conserve todo su estado y no dispare el
        # close-request (que apaga MPRIS). force_show_main se usa cuando el
        # minireproductor ya se cerró por su cuenta (p. ej. con su botón X)
        # y solo hace falta traer de vuelta la ventana principal.
        if force_show_main or self.mini_player.get_visible():
    ...
    def _on_file_chosen(self,dialog,result):
        ...
        if gfile and gfile.get_path():
            self.queue=[];self.queue_source=[];self.queue_index=-1;self.shuffle_history.clear();self.player_bar.load_file(gfile.get_path());self.player_bar.set_navigation_enabled(False)
            local_row={"title": Path(gfile.get_path()).stem, "path": gfile.get_path(), "artist": "Archivo local", "album": "", "cover_data": None}
            self.player_view.set_song(local_row)
            self.mini_player.set_song(local_row)
            self.mini_player.set_navigation_enabled(False)
            self._show_player_view()
