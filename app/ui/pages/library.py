...
class LibraryPage(Gtk.Box):
    ...
    def _refresh(self):
        ...
        for folder in folders:
            ...
            available = False
            try: available = root.exists() and root.is_dir()
            except OSError: pass
            row = Adw.ActionRow(title=folder)
            row.set_subtitle("Disponible" if available else "No disponible — se conservarán las canciones hasta que vuelva a montarse")
